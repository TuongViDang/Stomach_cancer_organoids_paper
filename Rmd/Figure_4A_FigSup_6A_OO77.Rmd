---
title: "Stomach cancer paper Figure_4A FigSup_6A patient OO77"
author: "Vi Dang"
date: "`r Sys.Date()`"
output:
  html_document:
...
plot
ggsave(file.path(plot_path,paste0("Figure_4A_Dimplot_by_sample_",patient,".pdf")),width = 10, height= 8)
```
...
ggsave(file.path(plot_path,paste0("Figure_4A_mapping_Clones_to_scRNA_",patient,".pdf")),width = 10, height= 8)
...
pheatmap(mutation_matrix_ordered, color = c("whitesmoke","red"),
        ...
        #,filename = file.path(plot_path,paste0( "Figure_Sup_6A_mutation_matrix_", patient,".Sorted.png")), width = 6,height=6,  units = "in", res = 600
       )
# Load the heatmap image
heatmap_img <- image_read(file.path(plot_path, paste0( "Figure_Sup_6A_mutation_matrix_", patient,".Sorted.png")), density = 600)
# Apply Gaussian Blur
blurred_img <- image_blur(heatmap_img, radius = 10, sigma = 1.5)

# Save the blurred image
image_write(blurred_img, path = file.path(plot_path, paste0( "Figure_Sup_6A_mutation_matrix_", patient,".Sorted.Blurred.pdf")),  density = 600,format = "pdf")

#Save Figures data
write.table(mutation_matrix_ordered, file.path(figures_data, paste0( "Figure_Sup_6A_mutation_matrix_", patient,".Sorted.txt")), sep = "\t", quote= F)
write.table(anno_mut_ordered,  file.path(figures_data, paste0( "Figure_Sup_6A_mutation_matrix_annotation_mutation_", patient,".Sorted.txt")), sep = "\t", quote= F)
write.table(anno_cell_ordered,  file.path(figures_data, paste0("Figure_Sup_6A_mutation_matrix_annotation_cell_", patient, ".Sorted.txt")), sep = "\t", quote= F)
```
...
pheatmap(mut_scale_mat, colorRampPalette(rev(brewer.pal(n = 11, name ="RdBu"))[3:9])(7),
        ...
        annotation_col = anno_cell_ordered, annotation_colors = Clone_colors ,
        gaps_row = rows_gap , gaps_col = cols_gap 
        #,filename = file.path(plot_path,paste0( "Figure_Sup_6A_expression_matrix_", patient,".Sorted.png")), width = 6,height=6
        )

#Save data
write.table(mut_scale_mat, file.path(figures_data, paste0( "Figure_Sup_6A_expression_matrix_", patient,".Sorted.txt")), sep = "\t", quote= F)
...
pheatmap(mutation_matrix_ordered_CS, color = c("whitesmoke","red"),
        ...
        legend_breaks = c(0, 1),
        gaps_row = rows_gap , gaps_col = cols_gap 
     #,filename = file.path(plot_path,paste0( "Figure_4A_mutation_matrix_", patient,".Sorted.png")), width = 6,height=6,  units = "in", res = 600
       )
# Load the heatmap image
heatmap_img <- image_read(file.path(plot_path, paste0( "Figure_4A_mutation_matrix_", patient,".Sorted.png")), density = 600)
# Apply Gaussian Blur
blurred_img <- image_blur(heatmap_img, radius = 10, sigma = 1.5)

# Save the blurred image
image_write(blurred_img, path = file.path(plot_path, paste0( "Figure_4A_mutation_matrix_", patient,".Sorted.Blurred.pdf")),  density = 600,format = "pdf")
```
...
# Save the plot
ggsave( file.path(plot_path, paste0("Figure_Sup_6A_Purity_cell_clone_",patient,".pdf")), width = 11, height = 5, scale =1)

#save Figure data
write.table(Purity, file.path(figures_data, paste0("Figure_Sup_6A_Purity_cell_clone_", patient,".txt")), sep = "\t", quote = F)
...
ggsave(file.path(plot_path,paste0( "Figure_Sup_6A_Fraction_mutation_clone_in_cell_clone_",patient,".pdf")), width = 11, height = 12.5, scale=1)

#save Figure data
colnames(Contaminate_cell_each_clone ) = c("Cell_clone", "Mutation_clone", "Fraction")
write.table(Contaminate_cell_each_clone, file.path(figures_data,paste0( "Figure_Sup_6A_Fraction_mutation_clone_in_cell_clone_",patient,".txt")), sep = "\t", quote = F)
...
ordered_samples = c("OO77-no", "OO77-vitro", "OO77-vivo")
```
...
plot
# Save the plot
ggsave(file.path(plot_path, paste0( "Figure_4A_clone_proportion_", patient,".pdf")), width = 8 , height = 8, scale = 1)
...
write.table(data, file.path(figures_data,paste0( "Figure_4A_clone_proportion_", patient,".txt")), sep = "\t", quote = F)
```
